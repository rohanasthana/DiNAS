...
#allowed_bonds= {'INP': 1, '1x1C': 1, '3x3C': 1, '3X3M':1, 'OUT':1}
bond_dict = [None, Chem.rdchem.BondType.SINGLE]
...
def build_molecule(atom_types, edge_types, atom_decoder, verbose=False):
    ...
    mol = Chem.RWMol()
    conv = {'output': 'C', 'input': 'N', 'input_1': 'O', 'identity': 'F', 'max_pool_3x3': 'B', 'avg_pool_3x3':'Al','sep_conv_3x3':'I','sep_conv_5x5':'S','dil_conv_3x3':'Cl','dil_conv_5x5':'As','skip_connect':'Br'}
    for atom in atom_types:
        a = Chem.Atom(conv[atom_decoder[atom.item()]])
        mol.AddAtom(a)
        if verbose:
            print("Atom added: ", atom.item(), conv[atom_decoder[atom.item()]])
...
def check_stability(atom_types, edge_types, dataset_info, debug=False,atom_decoder=None):
    ...
    for atom_type, atom_n_bond in zip(atom_types, n_bonds):
        possible_bonds = allowed_bonds[conv[atom_decoder[atom_type]]]
        if type(possible_bonds) == int:
            is_stable = possible_bonds == atom_n_bond
        else:
            is_stable = atom_n_bond in possible_bonds
        if not is_stable and debug:
            print("Invalid bonds for molecule %s with %d bonds" % (conv[atom_decoder[atom_type]], atom_n_bond))
