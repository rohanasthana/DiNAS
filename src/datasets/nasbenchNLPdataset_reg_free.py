...
from src.NASBenchNLP_self import Dataset
...
class NASBenchDataset_NLP(Dataset):
    def __init__(self, data_file):
        ...
        for i in self.data:
            ...
            ##print(adj)
            adj=to_dense_adj(i.edge_index)[0]
            ...
            '''adj_normal=to_dense_adj(i.edge_index)[0]
            ...
            self.adjs.append(torch.tensor(self.newadj))
    ...
    def __getitem__(self, idx):
        ...
        types = {'out': 0, 'in': 1, 'sigm': 2, 'tanh': 3, 'lrelu': 4, 'sum':5, 'prod':6,'linear':7,'blend':8}
        #type_idx=[]
        ...
        X = F.one_hot(torch.tensor(self.ops[idx]), num_classes=len(types)).float()
...
class NASBenchNLPDataset(NASBenchDataset_NLP):
    def __init__(self,dataset_choice):
        super().__init__('/home/asthana/Documents/DiGress/DiGress/src/datasets/NASBenchNLP/cache_acc')
        
        
class NASBenchNLPDataModule(MolecularDataModule):
    ...
    def prepare_dataset(self,cfg):
        ...
        graphs = NASBenchNLPDataset(self.dataset_choice)
        test_len=1
...
class NASBenchNLPDatasetInfos(AbstractDatasetInfos):
    def __init__(self, datamodule, cfg):
            #self.atom_encoder = {'INP': 0, '1x1C': 1, '3x3C': 2, '3X3M':3, 'OUT':4}
            self.atom_encoder ={'out': 0, 'in': 1, 'sigm': 2, 'tanh': 3, 'lrelu': 4, 'sum':5, 'prod':6,'linear':7,'blend':8}
                         
            
            self.atom_decoder = ['out', 'in', 'sigm', 'tanh', 'lrelu', 'sum', 'prod','linear','blend']
