...
import wandb
import os
from datasets.nasbench101_dataset_regressor import NASBenchDataset,NASBenchDatasetInfos, NASBenchDataModule
...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    ...
    def test_epoch_end(self, outs) -> None:
        ...
        samples = []
        regressor_model = NASBenchRegressorDiscrete.load_from_checkpoint(os.path.join(self.cfg.general.regressor_path))
        ...
        while samples_left_to_generate > 0:
            ...
            for i in range(0,50):
                item=self.sample_batch(id, to_generate, num_nodes=None, save_final=to_save,
                                                 keep_chain=chains_save, number_chain_steps=self.number_chain_steps)
                #samples.extend(item)
                #print(len(item))
                operations = item[0][0]
                #print(operations)
                X = F.one_hot(torch.tensor(operations), num_classes=5).float()
                adj_matrix =item[0][1]
                n = adj_matrix.shape[-1]
                #adj_matrix=np.triu(np.array([list( map(int,x) ) for x in adj_matrix]))
                edge_index, _ = torch_geometric.utils.dense_to_sparse(torch.tensor(adj_matrix))
                ...
                data = torch_geometric.data.Data(x=X, edge_index=edge_index, edge_attr=edge_attr,
                                     y=y, idx=0,n_nodes=num_nodes)
                
                
                dense_data, node_mask = utils.to_dense(data.x, data.edge_index, data.edge_attr, data.batch)
                dense_data = dense_data.mask(node_mask)
                X, E = dense_data.X, dense_data.E
                pred = regressor_model.model(X, E, y, node_mask)
                print('PREDICTION REGRESSOR',pred.y)
                preds.append(pred.y)
                items.append(item)
            
            samples.extend(items[np.argmax(preds)])
