...
class GraphTransformerImageNet(nn.Module):
    """
    ...
    def __init__(self, n_layers: int, input_dims: dict, hidden_mlp_dims: dict, hidden_dims: dict,
        ...
        self.mlp_in_y = nn.Sequential(nn.Linear(2*input_dims['y'], hidden_mlp_dims['y']), act_fn_in,
        ...
        self.mlp_out_y = nn.Sequential(nn.Linear(hidden_dims['dy'], hidden_mlp_dims['y']), act_fn_out,
                                       nn.Linear(hidden_mlp_dims['y'], 2*output_dims['y']))
        
        self.linear_map_class_X = nn.Sequential(
            nn.Linear(input_dims['y'], hidden_dims['dx']),
            nn.ReLU(),
            nn.Linear(hidden_dims['dx'], int(hidden_dims['dx']/2)),
        ...
        self.linear_map_class_E = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dims['de'], int(hidden_dims['de']/2)),
    ...
    def forward(self, X, E, y, node_mask):
        ...
        #y=torch.unsqueeze(y,dim=-1)
        #print('Y',y)
        new_y=y.reshape(y.shape[0],(y.shape[1]*y.shape[2]))
        new_y=self.mlp_in_y(new_y)
        ...
        y_lr=y[:,:,0].reshape((y.shape[0],2))
        y_ntk=y[:,:,1].reshape((y.shape[0],2))
        
        y_X_lr = self.linear_map_class_X(y_lr)
        y_E_lr = self.linear_map_class_E(y_lr)
        y_X_ntk = self.linear_map_class_X_lat(y_ntk)
        y_E_ntk = self.linear_map_class_E_lat(y_ntk)
        #print('Y_X',y_X.shape)
        #print('Y_E',y_E.shape)
        #print(y.shape)
        y_X_ntk = y_X_ntk.view(int(y_X_ntk.shape[0]),1,y_X_ntk.shape[1])

        y_X_ntk = y_X_ntk.expand(-1, X.shape[1], -1)
        #print(y_X.shape)
        y_E_ntk = y_E_ntk.view(int(y_E_ntk.shape[0]),1,1,y_E_ntk.shape[1])

        y_E_ntk = y_E_ntk.expand(-1, E.shape[1],E.shape[2], -1)
        ...
        y_X_lr = y_X_lr.view(int(y_X_lr.shape[0]),1,y_X_lr.shape[1])

        y_X_lr = y_X_lr.expand(-1, X.shape[1], -1)
        #print(y_X.shape)
        y_E_lr = y_E_lr.view(int(y_E_lr.shape[0]),1,1,y_E_lr.shape[1])

        y_E_lr = y_E_lr.expand(-1, E.shape[1],E.shape[2], -1)
        #print(y_E.shape)
        ...
        #print(y_X_lr.shape)
        #print(y_E_lr.shape)
        y_X=torch.cat([y_X_lr,y_X_ntk],dim=-1)
        #print(y_X.shape)
        y_E=torch.cat([y_E_lr,y_E_ntk],dim=-1)
        ...
        #print(y.shape,y_to_out.shape)
        
        y = y.reshape(y.shape[0],int(y.shape[-1]/2),int(y.shape[-1]/2)) + y_to_out
